...
-- i.e.: "sara_web_store.customers"
INSERT INTO sara_web_store.customers VALUES (DEFAULT,'Pepa','García','1985-11-04','josefa@hotmail.com',NULL,DEFAULT);
...
-- IN operator
SELECT * FROM sara_web_store.customers
...
SELECT * FROM
	(SELECT o.order_id, `date`, first_name, last_name, 
		   p.`name` AS product, cl.`name` AS color, 
		   z.`name` AS size, quantity, unity_price
	FROM sara_web_store.orders AS o
		JOIN sara_web_store.order_description AS od
			ON o.order_id = od.order_id
		JOIN sara_web_store.customers AS c
			ON c.customer_id = o.customer_id
		JOIN sara_stock.products AS p
			ON p.product_id = od.product_id
		JOIN sara_stock.colors AS cl
			ON cl.color_id=od.color_id
		JOIN sara_stock.sizes AS z
			ON od.size_id=z.size_id) AS orders_web
RIGHT JOIN
	(SELECT pr.`name`,c.name AS color, sz.name AS size, quantity
	FROM products AS pr
		JOIN inventory AS inv
		...
		JOIN sizes AS sz
			ON inv.size_id = sz.size_id
	WHERE inv.warehouse_id = 1) AS stock
ON
	orders_web.product = stock.name AND
    orders_web.size = stock.size AND
    orders_web.color = stock.color ;


-- Temporary Table
USE sara_stock;

CREATE TEMPORARY TABLE `web_orders`
SELECT o.order_id, `date`, first_name, last_name, 
					   p.`name` AS product, cl.`name` AS color, 
					   z.`name` AS size, quantity, unity_price
				FROM sara_web_store.orders AS o
...
CREATE TEMPORARY TABLE w1_stock
SELECT pr.`name`,c.name AS color, sz.name AS size, quantity
					FROM products AS pr
						JOIN inventory AS inv
							ON pr.product_id = inv.product_id
...
LEFT JOIN
		w1_stock
ON
	web_orders.product = w1_stock.name AND
    web_orders.size = w1_stock.size AND
    web_orders.color = w1_stock.color ;
