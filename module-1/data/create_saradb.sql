...
DROP SCHEMA IF EXISTS `sara_stock`;
DROP SCHEMA IF EXISTS `sara_web_store`;
DROP SCHEMA IF EXISTS `sara_organization`;
SET FOREIGN_KEY_CHECKS=1;

CREATE SCHEMA IF NOT EXISTS `sara_web_store` DEFAULT CHARACTER SET UTF8MB4 ;
USE `sara_web_store` ;

CREATE TABLE IF NOT EXISTS `sara_web_store`.`customers` (
...
CREATE TABLE IF NOT EXISTS `sara_web_store`.`orders` (
  ...
  CONSTRAINT `fk_orders_customers`
    ...
    REFERENCES `sara_web_store`.`customers` (`customer_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE SCHEMA IF NOT EXISTS `sara_stock` DEFAULT CHARACTER SET UTF8MB4 ;
USE `sara_stock` ;

CREATE TABLE IF NOT EXISTS `sara_stock`.`category` (
  `category_id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(100) NOT NULL,
  PRIMARY KEY (`category_id`))
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `sara_stock`.`products` (
  ...
  CONSTRAINT `fk_products_category`
    ...
    REFERENCES `sara_stock`.`category` (`category_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `sara_web_store`.`social_media` (
  `customer_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `sara_web_store`.`addresses` (
...
CREATE TABLE IF NOT EXISTS `sara_web_store`.`reviews` (
  ...
  CONSTRAINT `fk_reviews_products`
    ...
    REFERENCES `sara_stock`.`products` (`product_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_reviews_orders`
    FOREIGN KEY (`order_id`)
    REFERENCES `sara_web_store`.`orders` (`order_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE SCHEMA IF NOT EXISTS `sara_organization` DEFAULT CHARACTER SET UTF8MB4 ;
USE `sara_organization` ;
CREATE TABLE IF NOT EXISTS `sara_organization`.`warehouses` (
...
CREATE TABLE IF NOT EXISTS `sara_organization`.`staff` (
  ...
  CONSTRAINT `fk_staff_office`
    FOREIGN KEY (`office`)
    REFERENCES `sara_organization`.`warehouses` (`warehouse_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `sara_stock`.`sizes` (
    `size_id` INT NOT NULL AUTO_INCREMENT,
    `name` VARCHAR(100) NOT NULL,
    PRIMARY KEY (`size_id`))
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `sara_stock`.`colors` (
    `color_id` INT NOT NULL AUTO_INCREMENT,
    `name` VARCHAR(100) NOT NULL,
    PRIMARY KEY (`color_id`))
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `sara_stock`.`inventory` (
  `product_id` INT NOT NULL,
  ...
  CONSTRAINT `fk_inventory_sizes`
    FOREIGN KEY (`size_id`)
    REFERENCES `sara_stock`.`sizes` (`size_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_inventory_colors`
    FOREIGN KEY (`color_id`)
    REFERENCES `sara_stock`.`colors` (`color_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
USE `sara_web_store` ;
CREATE TABLE IF NOT EXISTS `sara_web_store`.`order_description` (
